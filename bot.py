...
def escape_html(text):
    """Escape karakter HTML dari data eksternal seperti nama saham."""
...
def build_signal_msg(d, ind, scoring, tpsl, candle, ai_txt, modal):
    ...
    bar = ("🟢🟢 <b>SINYAL: BELI</b> 🟢🟢" if sinyal=="BELI" else
           "🔴🔴 <b>SINYAL: JUAL</b> 🔴🔴" if sinyal=="JUAL" else
           "🟡🟡 <b>SINYAL: HOLD</b> 🟡🟡")
    ...
    lines=[
        f"{'📈'if change>=0 else'📉'} <b>{d['symbol']}</b> — {escape_html(d['name'])}",
        f"💵 <b>Rp {fmt(price)}</b>  ({change:+.2f}%)  Vol:{fmt_vol(d['volume'])}",
    ...
    if tpsl["valid"]:
        lines+=[
            ...
            f"🛑 <b>STOP LOSS  :</b> Rp {fmt(tpsl['sl'])}  ({pct(tpsl['sl'],price):+.1f}%)",
            f"   Risk per lot: Rp {fmt(tpsl.get('risk',0)*100)}","",
        ]
        if lot>0:
            lines+=[
                f"💰 <b>SIMULASI MODAL Rp {fmt(modal)}:</b>",
                f"   Beli: <b>{lot} lot</b> ({lot*100:,} lbr) | Modal: Rp {fmt(modal_used)}",
                f"   💵 TP1: <b>+Rp {fmt(pl(tpsl['tp1']))}</b>",
                f"   💵 TP2: <b>+Rp {fmt(pl(tpsl['tp2']))}</b>",
                f"   💵 TP3: <b>+Rp {fmt(pl(tpsl['tp3']))}</b>",
                f"   💸 SL : <b>-Rp {fmt(abs(pl_sl()))}</b>","",
                "   📌 <i>TP1→50% | TP2→30% | TP3→20%</i>",
            ]
        else:
            lines.append(f"   ⚠️ Butuh min Rp {fmt(price*100)} untuk 1 lot")
    else:
        lines+=["━━━━━━━━━━━━━━━━━━━━",
                f"⚠️ <b>STATUS:</b> {tpsl.get('invalid_reason','')}"]

    lines+=["","━━━━━━━━━━━━━━━━━━━━","📊 <b>SUPPORT &amp; RESISTANCE:</b>"]
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "*📊 ANALISIS:*\n"
        "  <code>/sinyal BBCA</code> — sinyal + TP/SL berbasis S/R nyata\n"
        "  <code>/sinyal BBCA 2000000</code> — modal kustom\n\n"
        "*🔍 SCREENER:*\n"
        "  <code>/screener</code> — scan LQ45 cari yang bullish\n"
        "  <code>/screener 5</code> — top 5 saham terkuat\n\n"
        "*📈 BACKTEST:*\n"
        "  <code>/backtest BBCA</code> — uji akurasi sinyal 60 hari\n"
        "  <code>/backtest BBCA 30</code> — 30 hari\n\n"
        "*📋 JURNAL TRADING:*\n"
        "  <code>/beli BBCA 2 9500</code> — catat beli 2 lot @9500\n"
        "  <code>/jual BBCA 1 9800</code> — catat jual 1 lot @9800\n"
        "  <code>/jurnal</code> — lihat riwayat transaksi\n"
        "  <code>/posisi</code> — posisi aktif\n"
        "  <code>/statistik</code> — win rate & total P&L\n\n"
        "*🛠 TOOLS:*\n"
        "  <code>/average BBCA 2 9500 3 9000</code>\n"
        "  <code>/cutloss BBCA 1200 2</code>\n"
        "  <code>/portofolio</code>\n\n"
...
async def cmd_sinyal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        await update.message.reply_text("❌ Format: <code>/sinyal BBCA</code>", parse_mode="HTML"); return
    ticker = ctx.args[0].upper()
    modal  = 1_000_000
    if len(ctx.args)>=2:
        try: modal=int(ctx.args[1])
        except: pass
    msg = await update.message.reply_text(f"⏳ Menganalisis *{ticker}*...", parse_mode="HTML")
    try:
...
async def cmd_screener(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lines += [
        "━━━━━━━━━━━━━━━━━━━━",
        "💡 Analisis detail: <code>/sinyal BBCA</code>",
...
async def cmd_backtest(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        await update.message.reply_text("❌ Format: <code>/backtest BBCA</code> atau <code>/backtest BBCA 30</code>",
    ...
    msg = await update.message.reply_text(
        f"⏳ Backtest *{ticker}* selama {days} hari...", parse_mode="HTML")
    try:
...
async def cmd_beli(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ctx.args or len(ctx.args) < 3:
        await update.message.reply_text(
            "❌ Format: <code>/beli BBCA 2 9500</code> atau <code>/beli BBCA 2 9500 bottom fishing</code>",
            parse_mode="HTML"); return
    ticker = ctx.args[0].upper()
    try:
        lot   = int(ctx.args[1])
...
async def cmd_jual(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args or len(ctx.args) < 3:
        await update.message.reply_text(
            "❌ Format: <code>/jual BBCA 1 9800</code>", parse_mode="HTML"); return
    ticker = ctx.args[0].upper()
    try:
        lot   = int(ctx.args[1])
        price = float(ctx.args[2])
...
async def cmd_jurnal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not trades:
        await update.message.reply_text(
            "📋 Jurnal masih kosong.\nMulai catat: <code>/beli BBCA 2 9500</code>",
            parse_mode="HTML"); return
...
async def cmd_posisi(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not positions:
        await update.message.reply_text(
            "📊 Belum ada posisi aktif.\nCatat beli: <code>/beli BBCA 2 9500</code>",
            parse_mode="HTML"); return
...
async def cmd_average(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args or len(ctx.args) < 3:
        await update.message.reply_text("❌ Format: <code>/average BBCA 2 9500 3 9000</code>", parse_mode="HTML"); return
    ...
    msg = await update.message.reply_text(f"⏳ Mengambil harga *{ticker}*...", parse_mode="HTML")
    try:
        d = get_stock_data(ticker)
...
async def cmd_cutloss(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args or len(ctx.args)<2:
        await update.message.reply_text("❌ Format: <code>/cutloss BBCA 1200 2</code>", parse_mode="HTML"); return
    ticker=ctx.args[0].upper()
    ...
    msg = await update.message.reply_text(f"⏳ Analisis cut loss *{ticker}*...", parse_mode="HTML")
    try:
        ...
        if current<s1:       rek=f"⚠️ Di bawah S1 (Rp {fmt(s1)}). <b>Cut loss sekarang.</b>"
        elif current<buy*.93:rek=f"⚠️ Turun >7%. <b>Sangat disarankan cut loss.</b>"
        ...
        text=(f"✂️ <b>CUT LOSS — {ticker}</b>\n\n"
              f"📌 Harga Beli : Rp {fmt(buy)}\n"
              f"📌 Harga Skrg : Rp {fmt(current)}\n"
              f"📌 Posisi     : {lot} lot\n"
              f"📌 Status     : {status} ({pl_p:+.2f}%)\n"
              f"📌 P&L        : <b>{sign}Rp {fmt(abs(pl_val))}*\n"
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text.isalpha() and 2<=len(text)<=6:
        ...
        await update.message.reply_text(
            "❓ Ketik <code>/help</code> untuk panduan, atau kirim kode saham: *BBCA*",
            parse_mode="HTML")
