...
def get_data(ticker):
    ...
    try:
        ...
        logger.info(f"Fetching data for: {full}")
        stock = yf.Ticker(full)
        df = stock.history(period="3mo")
        logger.info(f"Got {len(df)} rows of data")
...
async def sinyal(update: Update, ctx):
    ...
    try:
        data = analyze(ticker)
        if not data:
            await update.message.reply_text(f"❌ Data tidak ditemukan untuk {ticker}\nCoba ticker lain (contoh: BBCA, BBRI, TLKM)")
            return
    except Exception as e:
        logger.error(f"Analysis error: {e}")
        await update.message.reply_text(f"❌ Error saat analisis: {str(e)}")
        return
    ...
    # Simple message - avoid complex emojis with markdown
    msg = f"📊 ANALISIS {ticker}\n\n"
    msg += f"💰 Harga: Rp {data['price']:,.0f} ({change_sign}{data['change']:.2f}%)\n"
    ...
    msg += f"• MA20: Rp {data['ma20']:,.0f}\n"
    msg += f"• MA50: Rp {data['ma50']:,.0f}\n"
    msg += f"• MA200: Rp {data['ma200']:,.0f}\n\n"
    msg += "━━━━━━━━━━━━━━━━━━━━\n\n"
    msg += f"📍 Support: Rp {data['support']:,.0f}\n"
    msg += f"🔴 Resistance: Rp {data['resistance']:,.0f}\n\n"
    msg += f"🎯 TP: Rp {data['tp']:,.0f}\n"
    msg += f"🛡️ SL: Rp {data['sl']:,.0f}\n"
    msg += f"📐 R/R: {data['rr']:.2f}x\n"

    if data['signal'] == 'BUY' and not data['tp_valid']:
        msg += "\n⚠️ PERHATIAN: R/R < 1.5x"
