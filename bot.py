...
def get_ai_summary(d, ind, scoring, tpsl):
    ...
    tp_info = (f"Entry:{fmt(tpsl['entry'])} SL:{fmt(tpsl['sl'])} TP1:{fmt(tpsl['tp1'])}(RR{tpsl['rr1']:.1f}x) "
               f"TP2:{fmt(tpsl['tp2'])}(RR{tpsl['rr2']:.1f}x) TP3:{fmt(tpsl['tp3'])}(RR{tpsl['rr3']:.1f}x)"
...
def build_signal_msg(d, ind, scoring, tpsl, candle, ai_txt, modal):
    ...
    for line in ai_txt.splitlines():
        ...
        if   mode=="k" and l: k+=l+" "
        elif mode=="s" and l: s+=l+" "
        elif mode=="w" and l: w+=l+" "
